...
for (let i = 0; i < dataset.length; i++) {
  for (let j = i + 1; j < dataset.length; j++) {
    ...
    if (!maxPair || sim > maxPair.sim) maxPair = { sim, a: dataset[i].file, b: dataset[j].file };
  }
}

pairs.sort((x, y) => y.sim - x.sim);

console.log(`Pages audited (article-only): ${dataset.length}`);
console.log(`Max similarity (5-gram Jaccard, article-only): ${(maxPair.sim * 100).toFixed(2)}%`);
console.log(`Most similar pair: ${maxPair.a} <> ${maxPair.b}`);
console.log('Top 10 similar article pairs:');
